...
Sphere::Sphere(double radius, Vector3d const& center, Matrix4d const& transform)
...
bool Sphere::Hit(Ray const& ray, Intersection & intersection)const
{
	...
	*/
	...
	auto addHit = [&ray, &invRay, this, &intersection](double t) {
		if (t > HIT_TIME_EPSILON)
		{
			...
			Vector3d hitPoint0 = ray.GetPointAtTime(t);
			Vector3d hitPoint0InObjectSpace = invRay.GetPointAtTime(t);
			...
			Vector3d hitNormal0 = GetNormalMatrix() * hitNormal0InObjectSpace;
