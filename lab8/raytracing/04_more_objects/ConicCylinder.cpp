...
ConicCylinder::ConicCylinder(
	...
	Matrix4d const& tranform)
	: GeometryObjectWithInitialTransformImpl(tranform)
{
	assert(height >= 0);
	assert(baseRadius >= 0);
	assert(capRadius >= 0);
...
bool ConicCylinder::Hit(Ray const& ray, Intersection& intersection) const
{
	...
	*/
	...
	for (unsigned i = 0; i < numHits; ++i)
	{
		...
		*/
		...
		{
			hitNormalInObjectSpace = Vector3d(hitPointInObjectSpace.x, hitPointInObjectSpace.y, -q * (1 + q * hitPointInObjectSpace.z));
		}
		else if (h.hitSurface == HIT_BASE)
		{
			hitNormalInObjectSpace = Vector3d(0, 0, -1);
		}
		else
		{
			hitNormalInObjectSpace = Vector3d(0, 0, 1);
