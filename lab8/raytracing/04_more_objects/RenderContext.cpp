...
void RenderContext::RenderRectangle(Scene const& scene, int left, int top, unsigned width, unsigned height, IColorBuffer & colorBuffer)const
{
	...
	Ray leftTop = CalculateRay(left + 0.5, top + 0.5);
	Ray rightTop = CalculateRay(right + 0.5, top + 0.5);
	Ray leftBottom = CalculateRay(left + 0.5, bottom + 0.5);

	// Шаг, с которым изменяется начальная координата луча вдоль горизонтальной оси экрана
	Vector3d rayStartHorizontalStep = (rightTop.GetStart() - leftTop.GetStart()) / (double)width;
	// Шаг, с которым изменяется начальная координата луча вдоль вертикальной оси экрана
	Vector3d rayStartVerticalStep = (leftBottom.GetStart() - leftTop.GetStart()) / (double)height;

	// Шаг, с которым изменяется направление луча вдоль горизонтальной оси экрана
	Vector3d rayDirectionHorizontalStep = (rightTop.GetDirection() - leftTop.GetDirection()) / (double)width;
	// Шаг, с которым изменяется направление луча вдоль вертикальной оси экрана
	Vector3d rayDirectionVerticalStep = (leftBottom.GetDirection() - leftTop.GetDirection()) / (double)height;
...
Ray RenderContext::CalculateRay(double screenX, double screenY)const
{
	...
	Vector2d pixelCenter = GetNormalizedViewportCoord(screenX + 0.5, screenY + 0.5);

	// Вычисляем начальную и конечную точки луча, проходящего через центр данного пикселя
	Vector3d rayStart = UnProject(pixelCenter.x, pixelCenter.y, 0);
	Vector3d rayEnd = UnProject(pixelCenter.x, pixelCenter.y, 1);

	// Направление трассируемого луча
	Vector3d rayDirection = rayEnd - rayStart;
...
std::uint32_t RenderContext::QuantizeColor(Vector4f const& color)
{
	...
	Vector4f clampedColor = Clamp(color, 0.0f, 1.0f);
...
Vector3d RenderContext::UnProject(double normalizedX, double normalizedY, double depth)const
{
	...
	*/
	...
	Vector4d pos = invModelViewProj * Vector4d(normalizedX, normalizedY, normalizedZ, 1);
